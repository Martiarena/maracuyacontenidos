...
if ($eliminar == "true") {
	$sqlEliminar = "SELECT cod_link FROM social_links ORDER BY orden";
	$sqlResultado = mysqli_query($enlaces,$sqlEliminar) or die('Consulta fallida: ' . mysqli_error($enlaces));
	$x = 0;
	while($filaElim = mysqli_fetch_array($sqlResultado)){
		$id_link = $filaElim["cod_link"];
		if ($_REQUEST["chk" . $id_link] == "on") {
			$x++;
			if ($x == 1) {
					$sql = "DELETE FROM social_links WHERE cod_link=$id_link";
				} else { 
					$sql = $sql . " OR cod_link=$id_link";
				}
		}
	}
...
<head>
    ...
    <style>
		@media only screen and (max-width: 760px), (min-device-width: 768px) and (max-device-width: 1024px)  {
			td:nth-of-type(1):before { content: "Botón Red Social"; }
                ...
                <div class="fluid">
					<div class="widget grid12">
						...
						<div class="widget-content">
                        	<a class="btn btn-blue" href="social-nuevo.php"><i class="fa fa-plus" aria-hidden="true"></i> Añadir nuevo</a>
                            <hr>
							<form name="fcms" method="post" action="">
                                <table id="portafolio" class="text-center" width="100%" border="1">
                                	<thead>
                                        <tr>
                                            <th width="30%" scope="col">Botón de Red social
                                    ...
                                    <?php
                       		 			$consultarSol = 'SELECT * FROM social_links ORDER BY orden';
                                    ...
                                    <tr>
                                        ...
                                        <td><a href="<?php echo $xLink; ?>" target="_blank"><i class="fa fa-search" aria-hidden="true" style="font-size:20px;"></i> Ver enlace</a></td>
                                        <td><?php echo $xOrden; ?></td>
                                        <td><strong>[<?php if($xEstado=="A"){ echo "Activo";}elseif($xEstado=="I"){ echo "Inactivo";} ?>]</strong></td>
                                        <td><a class="boton-eliminar" href="social-delete.php?cod_link=<?php echo $xCodigo; ?>"><i class="fa fa-trash" aria-hidden="true"></i></a></td>
                                        <td><a class="boton-editar" href="social-edit.php?cod_link=<?php echo $xCodigo; ?>"><i class="fa fa-pencil-square" aria-hidden="true"></i></a></td>
                                        <td>
